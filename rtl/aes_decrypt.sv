module aes_decrypt (
  ...
  // Pre-expanded AES-128 round keys for FIXED_KEY.
  localparam logic [127:0] ROUND_KEYS [0:NUM_ROUNDS] = '{
      FIXED_KEY,
    ...
    unique case (controller_state_reg)
      STATE_IDLE: begin
        ...
        if (cipher_valid) begin
          state_next            = add_round_key(ciphertext, ROUND_KEYS[NUM_ROUNDS]);
      ...
      STATE_RUN: begin
        ...
        if (round_idx_reg > 0) begin
          state_next     = inv_round(state_reg, ROUND_KEYS[round_idx_reg]);
          round_idx_next = round_idx_reg - 1'b1;
        end else begin
          plaintext_next        = inv_final_round(state_reg, ROUND_KEYS[0]);
