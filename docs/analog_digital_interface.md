...
- Current RTL uses asynchronous assertion at the top-level boundary
- Internal logic uses a synchronized reset release (`rst_n_sync`)
- Reset behavior is asynchronous assert, synchronous deassert

`serial_bit`
- One serial data bit presented to the digital backend per `clk`
- The active RTL samples `serial_bit` directly on `posedge clk`
- Therefore, the current backend assumes `serial_bit` is already synchronous to `clk`

## Upstream Requirements

The analog or mixed-signal source that drives `serial_bit` must guarantee:
- `serial_bit` meets setup and hold timing relative to `clk`
- one valid data bit is delivered per clock cycle
- packet preamble and payload timing are aligned to the same `clk` domain seen by the digital backend
...
- a synchronizer or retiming stage before `packet_rx`
...
- Expected dominant logic depth: inverse AES round datapath in [`aes_decrypt.sv`](/Users/krishnachemudupati/Projects/rf-secure-id-asic/rtl/aes_decrypt.sv)
